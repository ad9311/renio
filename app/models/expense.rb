...
class Expense < ApplicationRecord
  ...
  after_destroy :update_budget_total_expenses_on_destroy
  ...
  def update_budget_total_expenses(params:)
    result = Budgets::UpdateTotalExpenses.call(budget:, params:)
    raise "Failed to update budget total expenses: #{result.inspect}" unless result.success?
  end
end
